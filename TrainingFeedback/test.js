...
//         session.sendTyping();
//         customOperations.sendEmailToTMTeam(session, username + "- " + i18n.__('notSubmitting'), "Here is the reason-: " + response, false, fs);
...
//             });
//         session.send("Please type 'edit (question number)' to edit the feedback. For example- **edit 1** or **'submit'** to submit feedback");
//         firebaseOperations.updateLastSentMessageOfPendingFeedback(new Date().getTime(), session.userData.attendeeId,
//             session.userData.trainingId);
//     },
//     function (session, results) {
...
//
//
// // The dialog stack is cleared and this dialog is invoked when the user enters 'submit'.
...
//         });
//         customOperations.sendEmailToTMTeam(session, username + "- " + i18n.__('subject_feedback'), "Please check the feedback in attached file-: ", true, fs);
//     }
//
// }
//
